...
def test_detect_returns_true_when_limit_increase_requests_exceed_threshold() -> None:
    detector = LimitIncreaseDetector(threshold=0)

    snapshot = build_snapshot(
        limit_increase_request_count=1,
    )

    signal = detector.detect(snapshot)

    assert signal is not None
    assert signal.code is RiskSignalCode.LIMIT_INCREASE
    assert signal.severity is Severity.HIGH
    assert signal.observed_value == 1
    assert signal.threshold == 0


def test_detect_returns_false_when_limit_increase_requests_equal_threshold() -> None:
    detector = LimitIncreaseDetector(threshold=1)
