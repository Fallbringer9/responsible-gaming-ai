...
from responsible_gaming.domain.frequent_deposit_detector import FrequentDepositDetector
...
def test_detect_returns_true_when_deposit_count_exceeds_threshold() -> None:
    detector = FrequentDepositDetector(threshold=3)
    ...
    assert signal.code is RiskSignalCode.FREQUENT_DEPOSIT
    assert signal.severity is Severity.MEDIUM
    assert signal.observed_value == 4
    assert signal.threshold == 3


def test_detect_returns_false_when_deposit_count_equals_threshold() -> None:
    detector = FrequentDepositDetector(threshold=4)
...
def test_init_raises_when_threshold_is_negative() -> None:
    with pytest.raises(ValueError, match="threshold must not be negative"):
        FrequentDepositDetector(threshold=-1)
