...
def test_detect_returns_true_when_night_sessions_exceed_threshold() -> None:
    detector = NightSessionDetector(threshold=0)
    ...
    assert signal.code is RiskSignalCode.NIGHT_SESSION
    assert signal.severity is Severity.MEDIUM
    assert signal.observed_value == 1
    assert signal.threshold == 0


def test_detect_returns_false_when_night_sessions_equal_threshold() -> None:
    detector = NightSessionDetector(threshold=1)
...
def test_init_raises_when_threshold_is_negative() -> None:
    with pytest.raises(ValueError, match="threshold must not be negative"):
        NightSessionDetector(threshold=-1)
